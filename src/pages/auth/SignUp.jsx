...
const SignUp = () => {
	...
	const sendOtp = async (e) => {
		...
		try {
			...
			const response = await registration(data); // Make sure login function returns token
			if (response.success == true) {
				showToast('success', response.message)
				navigate('/sign-up-otp-page', { state: { emailOrPhone } }); // Redirect upon successful login
			} else {
							...
							<div className="border bg-white shadow border-gray-100 hover-border-main-600 transition-1 rounded-16 px-24 py-20 h-100 w-100">
								...
								<div className="mt-20">
									...
									<span style={{ fontSize: "12px" }}>
										(If you already Registered please clink on Sign In)
									</span>
								</div>
							</div>
						</div>
