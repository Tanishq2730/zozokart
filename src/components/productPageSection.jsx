...
const ProductPageSection = ()=> {
  ...
  const toggleFilter = (filterType, optionValue) => { // Receive optionValue
          ...
          {filtersData.map((filter) => (
            <div key={filter.title} className="filtercard">
              ...
              {filter.options.map((option) => {
                const isChecked = selectedFilters[filter.title]?.includes(option.value) || false;
                return (
                  <div key={option.value} className="form-check mb-2">
                    <input
                      ...
                      id={option.value}
                      checked={isChecked}
                      onChange={() => toggleFilter(filter.title, option.value)} // Pass option.value
                    />
