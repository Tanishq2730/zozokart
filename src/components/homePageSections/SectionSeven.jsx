...
const SectionSeven = ({ homePage }) => {
  console.log(homePage);
  const sectionSevenData = homePage?.sectionSeven || { productIds: [] };
  ...
  const settings = {
    dots: false,
    infinite: sectionSevenData?.productIds.length > 6,
    speed: 800,
    slidesToShow: 6,
    slidesToScroll: 1,
    autoplay: true,
  ...
  const slides = useMemo(
    () => {
      const arr = [...sectionSevenData.productIds];
      while (arr.length < 6) {
        arr.push({ _id: `placeholder-${arr.length}`, placeholder: true });
      }
      return arr;
    },
    [sectionSevenData.productIds]
  );
